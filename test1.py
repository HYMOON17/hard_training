...
class MAML():
    def __init__(self, model,inner_lr, meta_lr, K=10, inner_steps=1, tasks_per_meta_batch=5):
        ...
        self.meta_optimiser = Adam(self.weights, meta_lr)
...
if __name__ == '__main__':
    ...
    # MTL Parameters
    train_dataset = mydataset('data/test_imgs_mtl.mat')
    num_of_tasks = 20
    sample_per_task = np.floor(len(train_dataset)/num_of_tasks).astype(np.int64)

    parser = argparse.ArgumentParser('Model-Agnostic Meta-Learning (MAML)')

    parser.add_argument('--num_of_tasks', type=int, default=20, #train은 100
    ...
    model = DnCNN(1, 1)
    model.load_state_dict(torch.load("HSM_Results/noise2selfMAML.pt", map_location='cpu'))
